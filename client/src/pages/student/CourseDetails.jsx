...
const CourseDetails = () => {
  ...
  return courseData ? (
    <>
   <div className='flex md:flex-row flex-col-reverse gap-10 relative min-h-screen items-start justify-center md:justify-between md:px-24 lg:px-36 px-8 md:pt-30 pt-20 text-left'>
        <div className='absolute inset-0 w-full h-screen z-0
        bg-gradient-to-b from-cyan-100 via-cyan-50 to-white'></div>
        ...
        <div  className='max-w-xl z-10 text-gray-500'>
          ...
          dangerouslySetInnerHTML={{__html : courseData.courseDescription.slice(0,
           ...
           <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
                     ...
                     <div className='flex'>
                       {[...Array(5)].map((_, i)=>(<img key={i} src={i < Math.
                       floor(calculateRating(courseData)) ? assets.star : assets.star_blank} alt='' className='w-3.5 h-3.5' />
                       ))}
                     </div>
...
<p>
  ...
  {(courseData.enrolledStudents?.length || 0) > 1 ? ' students' : ' student'}
                     ...
                     cursor-pointer select-none' onClick={() => toggleSection(index)}>
                      <div className='flex items-center gap-2'>
                        <img className={`transform transition-transform ${openSections 
                          [index] ? 'rotate-180' : ' '}`}src={assets.down_arrow_icon} alt="arrow_icon" />
                        <p className='font-medium md:text-base text-sm'>{chapter.chapterTitle}</p>
                      </div>
                      <p className='text-sm md:text-default'>{chapter.chapterContent?.length || 0} lecture - {calculateChapterTime(chapter)}</p>
                    </div>
        ...
        {/* right column */}
<div className='w-full sm:w-[300px] md:w-[350px] lg:w-[380px] flex-shrink-0 z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white'>  
   {
        playerData ? <Youtube videoId={playerData.videoId} opts={{playerVars: {autoplay: 1}}} iframeClassName='w-full aspect-video' />

        :  <img src={courseData.courseThumbnail}  alt="course_thumbnail" className='w-full h-[180px] sm:h-[200px] md:h-[230px] object-cover' />
